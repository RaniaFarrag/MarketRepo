        ...
        <div class="subheader py-2 py-lg-12  subheader-transparent " id="kt_subheader">
            <div class=" container  d-flex align-items-center justify-content-between flex-wrap flex-sm-nowrap">
                ...
                <div class="d-flex align-items-center">
                    ...
                    <!--begin::Button-->
                    <a href="{{ route('usersalaries.create') }}" class="btn btn-success font-weight-bold  py-3 px-6 mr-2">
                                    ...
                                    <table class="table table-bordered text-center">
                                        ...
                                        @foreach($users as $k=>$user)
                                            ...
                                            <tr>
                                                ...
                                                <td>{{ $user->userSalary ? $user->userSalary->salary : '-' }}</td>
                                                {{--<td><a class="btn btn-success font-weight-bold" href="{{ route('users.edit' , $user->id) }}">{{ trans('dashboard.edit') }}</a></td>--}}
                                                <td>
                                                    <form method="post" action="{{ route('usersalaries.destroy' , $user->id) }}">
                                                        @method('DELETE')
                                                        @csrf
                                                        <button {{ $user->userSalary ? '' : 'disabled' }} onclick="return confirm('Are you sure?')" class="btn btn-danger mr-2"
                                                                type="submit"><i class="fa fa-trash p-0"></i></button>
                                                    </form>
                                                </td>
                                            </tr>
                                        @endforeach
